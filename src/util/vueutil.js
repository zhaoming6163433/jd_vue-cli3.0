...
export default{
  ...
  install(Vue,options)
  {
    ...
    Vue.filter("timehrhandle", function(time) {
	    return time==''||time==undefined?'':new Date(time).Format("hh:mm");
    });
    //年 月 日过滤器
    Vue.filter("timeymdhandle", function(time) {
	    return time==''||time==undefined?'':new Date(time).Format("yyyy年MM月dd日");
    });
    //年 月 日 时 分 秒过滤器
    Vue.filter("timeymdhmshandle", function(time) {
	    return time==''||time==undefined?'':new Date(time).Format("yyyy年MM月dd日 hh:mm:ss");
    });
    //是否
    Vue.filter("isyesorno", function(val) {
	    switch(val){
            case "0":return "否";
            case "1":return "是";
            default:return "";
    ...
    //当前状态
    Vue.filter("nowstate", function(val) {
	    switch(val){
            case "0":return "已上线";
            case "1":return "暂停";
            case "2":return "未上线";
    ...
    Vue.prototype.agomonthf= function (val) {
		if(val == '/'){
            return '<span>'+val+'</span>';
        }else if(val.indexOf('-')!=-1){
            return '<span style="color:#ED5566">'+val+' ↓<span>';
        }else{
            return '<span style="color:#00AD8A">'+val+' ↑<span>';
        }
    };
  }
}
